...
eval 数据集(`eval/queries_decompose.jsonl`,9 条,三类别):deep_search(多步)vs search_notes(单步)。

**整体 9 条**(混合三类别):

| 指标 | 基线(单步) | Agentic(多步) |
|---|---|---|
| recall@5(any) | 88.9% | 88.9% |
| **recall_all@5(全命中)** | **11.1%** | **33.3%** |
| MRR | 0.833 | 0.833 |

> 报告:[AgenticRAG-decompose eval](../../04-测试/AgenticRAG-decompose_2026-07-28_v0.1.0-44339fa.md)

### 按类别拆分(适用范围,关键)

整体 recall_all 11%→33% 被三类别稀释,须拆开看 decompose 到底对谁有效:

| 类别 | 条数 | 触发分解? | 子查询数 | 基线 recall_all | Agentic recall_all | 结论 |
|---|---|---|---|---|---|---|
| compare(2 方对比) | 5 | ✅ | 3–4 | 20% | **60%** | ✅ 有效:多步覆盖双方 |
| multi-entity(3 方关系) | 2 | ✅ | 6 | 0% | 0% | ⚠️ 触发但无增量(指标过苛) |
| multi-intent(「以及」) | 2 | ❌ | 1 | 0% | 0% | ❌ 不触发(门控局限) |

> 💡 recall(any) 88.9% 太宽松看不出差异,**recall_all(全命中)才体现多步价值**——评估指标必须匹配场景(对比类 = 双方都召回),否则「两者差不多」误判多步无用(第一轮踩过)。

### 结论

**decompose 的有效场景是 2 方对比**(compare recall_all 20%→60%,+40pp):单步常漏一方,多步分别检索 A/B 覆盖双方。**印证 Step 1 教训:agentic 做底层做不了的事(对比类覆盖双方)才有价值。**

两类边界(扩展验证发现):
- **3 方关系(multi-entity)**:decompose **正确触发**(6 子查询:3 个「是什么」+ 3 个两两关系),但 recall_all 无提升(0→0)——top-5 要同时含 3 个不同笔记的相关 chunk,物理上过苛(bge 单步/多步都难全命中)。分解没坏事,只是 recall_all 对 3 方过严。
- **多意图(multi-intent「以及…实现」)**:**不触发分解**(1 子查询走单步)——decomposer 防护①(检测对比词:对比/区别/关系/vs)不匹配「以及」,走单步 = 基线。注意:decomposer **能力上能拆多意图**(见「现状」段验证),但生产 deep_search 的防护①门控拦了——能力 vs 触发的落差。这是触发条件的**已知局限**(防过度分解的代价),不单独做扩展(性价比低)。
...
- **decompose 触发条件扩展**(已知局限,暂不做):multi-intent(「以及/和…实现」)不触发分解(防护①对比词门控漏),可加多意图词,但增过度分解风险、性价比低。
